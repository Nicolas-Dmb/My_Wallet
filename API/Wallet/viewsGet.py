...
from .models import Buy, Sells, Wallet, Asset, CryptoDetail, BourseDetail, Cash, CashDetail, Categories, RealEstate, RealEstateDetail, HistoricalPrice,HistoricalWallet, HistoricalCrypto,HistoricalBourse,HistoricalCash,HistoricalImmo
...
class AmountCategories(APIView):
    ...
    def get(self, request, format=None):
        ...
        if categorie in ['crypto','course','cash','all']:
            try:
                match categorie:
                    case 'crypto':
                        amounts = Categories.objects.filter(wallet=wallet, type='Crypto').first()
                    case 'bourse':
                        amounts = Categories.objects.filter(wallet=wallet, type='Bourse').first()
                    case 'cash':
    ...
    def get(self, request, format=None):
        ...
        if categorie in ['crypto','bourse']:
            ...
            for asset in assets:
                ...
                #Type 
                if categorie == 'crypto':
                    Detail = CryptoDetail.objects.filter(asset = asset)
                else:
                    Detail = BourseDetail.objects.filter(asset = asset)
...
class RevenuAnnuelImmo(APIView):
    ...
    def get(self, request, format=None):
        ...
        serializer = RevenuAnnuelImmoSerializer(immos)
        return Response(serializer)
