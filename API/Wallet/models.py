...
class Wallet(models.Model):
    ...
    def maj_amount(self):
        try:
            categories = Categories.objects.filter(wallet=self)
        except : 
            return "error sur les catégories"
        self.amount = 0
        self.date = timezone.now()
        for categorie in categories:
            self.amount += categorie.amount
        try:
...
class Categories(models.Model):
    ...
    def new_SubWallet(self, categorie, wallet):
        ...
        self.type = categorie
        self.save()
        subWallet = Categories.objects.get(wallet=wallet, type=categorie)
        subWallet.maj_SubWallet(categorie)
    ...
    def new_Cash(self, wallet):
        ...
        self.type = 'Cash'
        self.save()
        cash = Categories.objects.get(wallet=wallet, type='Cash')
        cash.maj_Cash()
...
class Asset(models.Model):
    ...
    def new_asset(self, ticker, number, name, wallet,date):
        ...
        try:
            categories = Categories.objects.get(wallet=self.wallet, type=self.category )
        except:
            Categories.new_SubWallet(self, self.category, wallet)
    ...
    def maj_asset(self, number, date):
        ...
        try:
            ...
            Categories.new_SubWallet(self, self.category, self.wallet)
        categories.maj_SubWallet(self, self.category)
    ...
    def new_buy(self, type='undifined', categories='undifined', country='undifined', sector='undifined', company='undifined'):
        ...
        if asset:
            if asset.api_know:
                ...
                asset.maj_asset_withoutAPI(self, self.price_buy, self.date_buy, self.number_buy)
            else:
                asset.maj_asset_withoutAPI(self, asset.actual_price, self.date_buy,self.number_buy)
            ...
            if response == False:#pour les asset dont l'api ne met pas à jour
                Asset.create_asset_withoutAPI(self.wallet,False, self.name, type, self.price_buy, self.currency, categories, country, sector, company, self.ticker, self.date_buy, self.number_buy)
...
class CashDetail(models.Model):
    ...
    def cash_maj_Amount(self, amount='undifined'):
        ...
        if amount is not 'undifined':
            ...
            self.amount += amount
        try:
            categorie = Categories.objects.get(wallet=self.wallet, type='Cash')
            categorie.maj_Cash()
        except:
            Categories.new_Cash(self.wallet)
